...
func (c *chittyChatClient) Subscribe(ctx context.Context, in *SubscribeRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[BroadcastMessage], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
...
type ChittyChat_SubscribeClient = grpc.ServerStreamingClient[BroadcastMessage]
...
func (UnimplementedChittyChatServer) Subscribe(*SubscribeRequest, grpc.ServerStreamingServer[BroadcastMessage]) error {
...
func _ChittyChat_Subscribe_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(ChittyChatServer).Subscribe(m, &grpc.GenericServerStream[SubscribeRequest, BroadcastMessage]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type ChittyChat_SubscribeServer = grpc.ServerStreamingServer[BroadcastMessage]
